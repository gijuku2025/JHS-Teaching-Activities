...
function renderChapterGrid() {
  ...
  pairings.forEach((chapter) => {
    ...
    card.addEventListener("click", () => startSession(chapter));
...
function buildPool(chapter) {
  return shuffle(chapter.sentences.map((s, i) => toItem(chapter.id, s, i)));
}

function startSession(chapter) {
  session = {
    chapter,
    pool: buildPool(chapter),
    results: new Map(),
    lastLayouts: new Map(),
    roundIndex: 0,
    roundState: null,
