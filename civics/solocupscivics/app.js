...
async function init() {
  try {
    const res = await fetch("data/social-studies.json");
    const ss = await res.json();
    pairings = ss.chapters || [];
  } catch (e) {
    chapterGrid.innerHTML =
      '<p class="chapter-card">Could not load the sentence data. Check that data/social-studies.json is in place.</p>';
  }
}
...
function renderChapterGrid() {
  ...
  pairings.forEach((chapter) => {
    const card = document.createElement("button");
    card.className = "chapter-card";
    card.innerHTML = `<h2>${chapter.name}</h2><p>${chapter.sentences.length} sentences</p>`;
    card.addEventListener("click", () => toggleChapter(chapter.id, card));
    chapterGrid.appendChild(card);
  });
}

function toggleChapter(id, card) {
...
function updateStartButtonState() {
  btnStartTraining.disabled = selectedChapterIds.size === 0;
}

btnStartTraining.addEventListener("click", () => {
  const chosen = pairings.filter((c) => selectedChapterIds.has(c.id));
  if (chosen.length > 0) startSession(chosen);
});

// ---------- Session / pool ----------

function toItem(chapterId, sentence, idx) {
  return {
    key: `${chapterId}:${idx}`,
    chapterId,
...
function buildPool(chapters) {
  ...
  chapters.forEach((chapter) => {
    chapter.sentences.forEach((s, i) => items.push(toItem(chapter.id, s, i)));
  });
  return items;
}
