...
const state = {
  ...
  roundNumber: 0,
  readerTurn: 0,         // increments each round, index = readerTurn % players.length
  pool: [],
  poolIndex: 0,
  roundScores: [],
  totalScores: [],
  roundHistory: [],      // [{ round: 1, scores: [0,0,...] }, ...]
...
// ---------- setup screen: dynamic player/team rows ----------

const playersContainer = document.getElementById("players-container");
const addPlayerBtn = document.getElementById("add-player-btn");

function renderPlayerInputs(existingValues = []) {
  ...
  for (let i = 0; i < playerCount; i++) {
    ...
    label.textContent = `Player/Team ${i + 1} name`;
    ...
    if (playerCount > MIN_PLAYERS) {
      ...
      removeBtn.setAttribute("aria-label", `Remove player/team ${i + 1}`);
      removeBtn.textContent = "\u00d7";
      removeBtn.addEventListener("click", () => removePlayerRow(i));
  ...
  addPlayerBtn.style.display = playerCount >= MAX_PLAYERS ? "none" : "inline-block";
}
...
addPlayerBtn.addEventListener("click", () => {
  ...
  playerCount += 1;
  renderPlayerInputs(values);
});

function removePlayerRow(indexToRemove) {
  const values = collectCurrentValues();
...
startGameBtn.addEventListener("click", () => {
  const names = collectCurrentValues().map(n => n.trim());

  if (names.some(n => n.length === 0)) {
    setupError.textContent = "Enter a name for every player or team.";
    return;
  }
  const lower = names.map(n => n.toLowerCase());
  if (new Set(lower).size !== names.length) {
    setupError.textContent = "Names need to be different from each other.";
    return;
  }
  ...
  state.readerOrder = names
    .map((_, i) => i)
    .sort((a, b) => names[a].toLowerCase().localeCompare(names[b].toLowerCase()));
  state.readerTurn = 0;
  state.roundNumber = 0;
...
function currentReaderIndex() {
  return state.readerOrder[state.readerTurn % state.players.length];
}
...
function startRound() {
  ...
  state.poolIndex = 0;
  state.roundScores = new Array(state.players.length).fill(0);
